...
def get_neighbors(matrix, r, c):
    ...
    #ensure if neighbors are within the boundaries
    if r > 0 and not matrix[r-1][c].processed:
        neighbors.append(matrix[r-1][c])
    if r < shape[0] - 1 and not matrix[r+1][c].processed:
        neighbors.append(matrix[r+1][c])
    if c > 0 and not matrix[r][c-1].processed:
        neighbors.append(matrix[r][c-1])
    if c < shape[1] - 1 and not matrix[r][c+1].processed:
        neighbors.append(matrix[r][c+1])
    return neighbors
